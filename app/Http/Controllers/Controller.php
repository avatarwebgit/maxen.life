...
class Controller extends BaseController
{
    ...
    public function send_sms_to_user_if_product_exist($previous_quantity, $new_quantity, $product_id)
    {
        ...
        foreach ($user_ids as $user_id) {
            ...
            if ($user_exists) {
                try {
                    ...
                    $user->notify(new ProductUpdateSms($user));
        ...
        public function visit_insurance()
    {
        $user_ip = $this->GetRealIp();
        $date = Carbon::now();
        $site_visit_exists = SiteVisit::where('ip', $user_ip)->where('date', $date->format('Y-m-d'))->exists();
        if (!$site_visit_exists) {
            SiteVisit::create([
                'ip' => $user_ip,
                'date' => $date
            ]);
