...
//==================================================== methods =======================================================
    ...
    public function edit($method)
    {
        ...
        if ($method == 'truck') {
            $DeliveryMethodAmount = DeliveryMethodAmount::where('method_id', 6)->get();
            return view('admin.deliveryMethods.truck.index', compact('DeliveryMethodAmount'));
        }
        if ($method == 'bus') {
            $DeliveryMethodAmount = DeliveryMethodAmount::where('method_id', 7)->get();
            return view('admin.deliveryMethods.bus.index', compact('DeliveryMethodAmount'));
        }
    }
     ...
     public function update(Request $request)
    {
        ...
        if ($request->has('image')) {
            $path = public_path(env('IMAGE_DELIVERYMETHOD_UPLOAD_PATH')) . $method->image;
            unlink_image_helper_function($path);
            $fileNameImage = generateFileName($request->image->getClientOriginalName());
            $request->image->move(public_path(env('IMAGE_DELIVERYMETHOD_UPLOAD_PATH')), $fileNameImage);
        } else {
            $fileNameImage = $method->image;
        }
        if ($validator->fails()) {
...
//==================================================== post method =======================================================
    ...
    {
        $rows = DeliveryMethodAmount::where('method_id', $request->method_id)->get();
