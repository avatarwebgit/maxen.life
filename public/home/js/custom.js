			...
			bySwiper: function ($id, $obj) {
				dsnGrid.convertToJQuery($id).each(function () {
					let $s = new Swiper($(this).find('.swiper-container'), {
						...
						speed: 1000,
						navigation: {
							nextEl: $(this).find('[data-cursor="بعدی"]'),
							prevEl: $(this).find('[data-cursor="قبلی"]'),
						},
						pagination: {
			...
			bySlick: function ($id, $obj) {
				dsnGrid.convertToJQuery($id).each(function () {
					$obj = $.extend(
						...
						{
							speed: 700,
							prevArrow: '<div data-cursor="قبلی"></div>',
							nextArrow: '<div data-cursor="بعدی"></div>',
							responsive: [
								{
									breakpoint: 992,
									settings: {
	...
	function loadMore() {
		...
		button.on('click', function () {
			let page = $(this).attr('data-page'),
				...
				progress_text_load_more = button.find(
					'.progress-text.progress-load-more'
				),
		...
		return {
			...
			ajaxSlider: function ($e) {
				let active = $e.parents('.slide-content'),
					...
					img = $(
						'.main-slider .slide-item[data-dsn-id="' + id + '"] .cover-bg'
					).first(),
			...
			dsnCreateElement: function ($e, $target, $letter, $targetLtter) {
				...
				if (title_move.hasClass('dsn-cutter'))
			...
			completeElement: function (elAjax) {
				...
				if (title_move.length) {
					...
					if (title.find('.dsn-word-wrapper').length) {
						let currTrans = title
							.find('.dsn-word-wrapper')
							.css('transform')
							.split(/[()]/)[1];
						if (currTrans) currTrans = currTrans.split(',')[5];
						if (currTrans)
							tl.staggerTo(
								dsnGrid.randomObjectArray(
	...
	function navMenu() {
		...
		return {
			...
			hamburgerOpen: function () {
				...
				tl.to(
					mainNav.find('ul.extend-container > li > a .dsn-meta-menu'),
					0.5,
	...
	function effectAnimate() {
		...
		return {
			...
			headerPages: function () {
				...
				// 			y: '100%',
				// 			autoAlpha: 0,
				// 			scale: heroImg.hasClass('header-scale-hero')
				// 				? 0.9
				// 				: heroImg.hasClass('header-no-scale-hero')
				// 				? 1
				// 				: 1.08,
				// 		},
				// 		0
				...
				let parallaxProject = dsnGrid
					.tweenMaxParallax(effectScroll, controller)
					.addParrlax({
						id: eHeaderProject,
						triggerHook: 0,
						tween: parallax,
			...
			headerProject: function () {
				...
				if (
					$(eHeaderProject).length <= 0 ||
					$(eHeaderProject).hasClass('dsn-end-animate')
				) {
					eHeaderProject = null;
					return false;
				}
				...
				if (heroImg.length > 0 && !dsnGrid.isMobile()) {
					parallax.to(heroImg, 2, { width: '40%', right: '60%' });
				}
				...
				if (heroTitle.length) {
					...
					if (heroDec.length)
						TweenLite.set(heroTitle.find('.dsn-word-wrapper , .metas'), {
							y: '+=' + heroDec.height(),
						});
					...
					parallax.staggerTo(
						dsnGrid.randomObjectArray(
							heroTitle.find('.dsn-word-wrapper'),
							1.5
						),
						1.5,
						{
				...
				if (scrollD.length) {
					parallaxProject.on('progress', function ($num) {
						TweenLite.to(
							$('[data-dsn-header="project"]').find('.scroll-d img'),
							0.3,
			...
			nextProject: function () {
				...
				if (heroTitle.length) {
					...
					TweenLite.set(heroTitle.find('.dsn-word-wrapper'), {
						y: 50,
						autoAlpha: 0,
					});
			...
			animateNumbers: function () {
				...
				$element.each(function () {
					...
					dsnGrid
						.tweenMaxParallax(effectScroll, new ScrollMagic.Controller())
						.addParrlax({
							id: this,
							reverse: false,
							triggerHook: 0.5,
					...
					let parallaxIt = dsnGrid
						...
						.addParrlax({
							...
							tween: TweenLite.to(sectionWork, 0.5, {
								force3D: true,
								x:
									(allSection.last().offset().left -
										allSection.last().width() * 0) *
									-1,
			...
			parallaxImg: function () {
				...
				$('[data-dsn-grid="move-up"]').each(function ($index) {
					...
					if (img.length > 0) {
						...
						let parallax,
							pers = dsnGrid.getData(
								img,
								'y',
								img.hasClass('has-opposite-direction') ? '-20%' : '30%'
							),
			...
			dsnScrollTop: function () {
				...
				TweenLite.to('.scroll-to-top', 1, { left: -100, autoAlpha: 0 });
				...
				parallaxIt.on('enter', function ($s) {
					TweenLite.to('.scroll-to-top', 1, { left: 20, autoAlpha: 1 });
			...
			moveSection: function () {
				...
				moveUp.each(function () {
					...
					if (_that.data('dsn-responsive') === 'tablet' && wind.width() < 992)
						return;
...
function slider() {
	...
	return {
		...
		initSlider: function () {
			...
			let dsn_slider_content = dsn_slider.find(
				'.dsn-slider-content > .content-slider'
			);
			let $parent = this;
			let $object = [];
			...
			return new Swiper('.main-slider .slide-inner', {
				...
				pagination: {
					...
					renderCustom: function (swiper, current, total) {
						$('.main-slider .dsn-controls .dsn-numbers span.full-number').html(
							dsnGrid.numberText(total)
						);
						TweenLite.to(
