        ...
        <div class="col-xl-12 col-md-12 mb-4 p-4 bg-white">
            ...
            <form action="{{ route('admin.articles.update', ['article' => $article->id]) }}" method="POST" enctype="multipart/form-data">
                ...
                <div class="form-row">
                    ...
                    <div class="form-group col-md-3">
                        <label for="productCategorySelect">دسته بندی محصولات</label>
                        <select id="productCategorySelect" name="product_category_id[]" class="form-control" data-live-search="true" multiple>
                            @foreach ($product_categories as $product_category)
                                <option
                                    {{ in_array($article->id,$product_category->Articles()->pluck('id')->toArray()) ? 'selected' : '' }} value="{{ $product_category->id }}">
                                    {{ isset($product_category->parent->name) ? $product_category->parent->name : $product_category->name }}
                                    {{ isset($product_category->parent->name) ? '/'.$product_category->name : '' }}
                                </option>
                            @endforeach
                        </select>
                    </div>
