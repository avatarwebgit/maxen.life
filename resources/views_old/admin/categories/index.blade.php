...
@section('script')
    <script>
        ...
        function saleActive(category_id) {
            let selector = '#saleActive_' + category_id;
            $.ajax({
                url: "{{ route('admin.category.sale_active') }}",
                data: {
                    _token: "{{ csrf_token() }}",
                    category_id: category_id,
                },
            ...
            <div class="row d-lg-flex justify-content-between align-items-center">
                ...
                <div class="col-md-4 col-12">
                    <div class="d-lg-flex justify-content-end align-items-center">
                        <div>
                            <a class="btn btn-sm btn-success" href="{{ route('admin.measures.index') }}">
                        ...
                        <tr>
                            ...
                            <th>
                                @if(count($category->children)==0 and $category->parent_id!=0)
                                    <a href="{{ route('admin.category.peyk.price',['category'=>$category->id]) }}" class="btn btn-danger btn-sm">
                            ...
                            <th>
                                @if(count($category->children)==0 and $category->parent_id!=0)
                                    <a href="{{ route('admin.category.send_setting.edit',['category'=>$category->id]) }}" class="btn btn-warning btn-sm">
                            ...
                            <th>
                                <a title="فعال بودن فروش" id="saleActive_{{ $category->id }}" onclick="saleActive({{ $category->id }})"
                                   class="btn btn-sm {{ $category->is_sale==1 ? 'btn-success text-white' : 'btn-dark' }}">
                                    {{ $category->is_sale==1 ? 'فعال' : 'غیرفعال' }}
                                </a>
                            </th>
                            <th>
